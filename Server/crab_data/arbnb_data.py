...
def get_houses_brife_infro__in_city(city):
    ...
    if 'house_count' not in houses_information.keys():
        if len(houses_information) >= 300:
            houses_information['house_count'] = len(houses_information)
            write_json_file(file_name,houses_information)
            return houses_information,host_information
            #houses_information['house_count'] = len(houses_information)
        house_count = int(random.uniform(80,120))
        houses_information['house_count'] = house_count
    else:
        if len(houses_information) >= 80:
            house_count = houses_information['house_count']
        else:
            house_count = int(random.uniform(80,120))


    if len(houses_information) >= house_count:
        return houses_information,host_information

    query_city = city_querys[city][0]
    query_id = city_querys[city][1]
    
    cityURL = '&items_per_grid='+ str(house_count) \
                +'&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en-AU&luxury_pre_launch=false&metadata_only=false&place_id='\
                + str(query_id) + \
                '&query='\
                + str(query_city) +\
...
def house_detail_infro(city,houses_information):
    for key in houses_information:
        ...
        pages = int(reviews_count)
        #pages = int(reviews_count / 7 )

        if reviews_count > 0:
            houses_information = house_reviews(city,house_id,houses_information,reviews_count,key)
        else:
...
def house_reviews(city,house_id,houses_information,reviews_count,key):
    reviewsUrlbase = 'https://www.airbnb.com.au/api/v2/homes_pdp_reviews?currency=AUD&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en-AU&listing_id='\
            + str(house_id)+  \
            '&_format=for_p3'\
            +'&limit='+str(reviews_count) +'&offset='
    ...
    reviewsUrl = reviewsUrlbase  + str(0) \
...
def update_section():
    for city in city_querys:
        ...
        #print(city)
        if 'house_count' not in houses_information.keys():
            houses_count = len(houses_information)
        else:
            houses_count = houses_information['house_count']

        query_city = city_querys[city][0]
        query_id = city_querys[city][1]
        cityURL = '&items_per_grid='+ str(houses_count) + \
                    '&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en-AU&luxury_pre_launch=false&metadata_only=false&place_id='\
                    + str(query_id) + \
                    '&query='\
                    + str(query_city) +\
...
def merge_house_data():
    for i in city_querys:
        filename = i + '.json'
        houses_1 = read_json_file(filename)
        houses_2  = read_json_file('json_data/'+filename)
        for house_id in houses_2:
            if house_id not in houses_1.keys():
                houses_1[house_id] = houses_2[house_id]
            else:
                if len(houses_1[house_id]) == 18:
                    houses_1[house_id] = houses_2[house_id]
                    print(i,house_id)
    write_json_file(filename,houses_1)
        #print(i,len(houses_2))          
...
if __name__ == "__main__":
    #update_section()
    #house_reviews(city,house_id,houses_information,pages,key)
    #merge_house_data()
     for i in city_querys:
