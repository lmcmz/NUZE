...
from arbnb_data import city_querys,MonthToNumber,MonthDays
...
with open('create_tables.sql','w') as sql:
    ...
    print("  `client_name` varchar(100),",file = sql)
...
with open('houses.sql','w') as sql:
    ...
    for city in house_information:
        ...
        for house in house_information[city]:
            ...
            print(city,house_id)
            if house_information[city][house][4] is None:
                bedrooms = 0
            else:
            ...
            if len(house_information[city][house]) == 18:
                reviews_count = 0
            else:               
            ...
            print("INSERT IGNORE INTO `host_to_houses` (`host_id`, `house_id`) VALUES ",file = sql)
            ...
            if len(house_information[city][house]) == 18:
                ...
                for mm in MonthToNumber:
                    if mm == 'Oct':
                        continue
                    else:
                        month = MonthToNumber[mm]
                        dd = MonthDays[str(month)]
                        for date in range( 1,dd + 1 ):
                            if date < 10:
                                date = '0' + str(date)
                            else:
                                date = str(date)
                            dates = '2019-0' + str(month) + '-' + date
                            vacant = random.randint(0,2)
                            if vacant == 0:
                                available = False
                            else:
                                available = True
                            if date == '30' and  str(month) == '9':
                                print("({},\"{}\",{},\"{}\");".format(house_id,dates,price,available),file = sql)
                            else:
                                print("({},\"{}\",{},\"{}\"),".format(house_id,dates,price,available),file = sql)
